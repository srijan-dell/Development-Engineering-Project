...
% Define square wave amplitude (5 µA)
square_amp = 5e-6; % 5 microamperes
...
% Constructing step-hold with conditional perturbation
for i = 2:N
    ...
    % Check if i is of the form n*T/8 where n is an odd integer
    if mod(8*i/T, 2) == 1  % (8*i/T gives n, check if n is odd)
        step_x1(2*i - 1) = history_x1(i) + square_amp; % +5 µA perturbation
    else
        step_x1(2*i - 1) = history_x1(i); % No perturbation (0 µA)
    end
end

% Plot
figure('Color','w');
plot(step_time*1e3, step_x1, 'r','LineWidth',1.5); % Red line like Image 1
xlabel('Time (ms)');
ylabel('Current, x_1 (A)');
grid on;
legend('Step-Hold with Conditional Square Perturbation');
