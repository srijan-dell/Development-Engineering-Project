...
% Build step arrays for Ipd instead of Idc
step_time = zeros(1, 2*N - 1);
step_Ipd  = zeros(1, 2*N - 1);

% First step
step_time(1) = iteration_t(1);
step_Ipd(1)  = Ipd_func(history_x1(1));

for i = 2:N
    step_time(2*i - 2) = iteration_t(i);
    step_Ipd(2*i - 2)  = Ipd_func(history_x1(i-1));

    step_time(2*i - 1) = iteration_t(i);
    step_Ipd(2*i - 1)  = Ipd_func(history_x1(i));
end

% Plot Ipd vs Time
figure('Color','w');
plot(step_time*1e3, step_Ipd, '-o','LineWidth',1.5);
xlabel('Time (ms)');
ylabel('I_{pd} (A)');
title('Step-Hold Plot of I_{pd} vs. Time (Coarse + Fine Tuning)');
grid on;
...
xlabel('Time (ms)');
ylabel('I_{dc} (A)');
title('Step-Hold Plot of Idc vs. Time (Coarse + Fine Tuning)');
grid on;
